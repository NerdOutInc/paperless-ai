...
banner() {
    echo
    echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}  Paperless AI Setup${NC}"
...
collect_fresh_config() {
    echo "  Let's set up your Paperless AI instance."
    divider
...
services:
  ...
  companion:
    ...
    environment:
      ...
      SEMANTIC_ELBOW_MIN_RESULTS: "4"
      SEMANTIC_ELBOW_MIN_GAP: "0.025"
      SEMANTIC_ELBOW_DROP_RATIO: "0.88"
...
# Paperless AI configuration -- generated by installer
...
do_addon_install() {
    ...
    step "Pulling Paperless AI companion image..."
    ...
    if [[ "$PORT_80_IN_USE" == "true" ]] || [[ -n "${DOMAIN:-}" && "$PORT_443_IN_USE" == "true" ]]; then
        ...
        warn "Paperless AI will still expose the companion directly on port 3001."
        warn "Add /search to your existing reverse proxy when you are ready for same-origin search."
        ADDON_ENABLE_CADDY=false
    fi

    # Generate docker-compose.override.yml
    local override_content="# Paperless AI add-on -- generated by installer
...
ensure_caddy_for_legacy_no_domain_addon() {
    ...
    if [[ ! -f docker-compose.override.yml ]] \
        || ! grep -Eq 'Paperless (Ag|AI) add-on' docker-compose.override.yml; then
...
echo "Pulling Paperless AI companion image..."
...
print_addon_summary() {
    ...
    echo
    echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}  Paperless AI installed!${NC}"
    echo -e "${BOLD}════════════════════════════════════════════════════${NC}"
    echo
...
main() {
    ...
    if detect_paperless; then
        ...
        echo "  How would you like to install Paperless AI?"
        echo
